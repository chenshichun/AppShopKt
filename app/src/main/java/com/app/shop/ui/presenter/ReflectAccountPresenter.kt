 ...
 *
 */
class ReflectAccountPresenter : BasePresenter<ReflectAccountContract.View>(),
    ReflectAccountContract.Presenter {
    ...
    override fun walletSet(walletSetReq: WalletSetReq) {
        CoroutineScope(Dispatchers.Main).launch {
            mView!!.showLoading()
            val response = ApiRequest.create(HomeService::class.java).walletSet(walletSetReq)
            mView!!.hideLoading()
            if (response.body() == null) {
                ToastUtil.showNoNetworkToast()
            } else {
                if (response.body()!!.code == Constants.WEB_RESP_CODE_SUCCESS) {
                    mView!!.walletSet(response.body()!!)
                } else {
                    ToastUtil.showToast(response.body()!!.msg.toString())
                }
            }
